...
export default function VocabTracker() {
  ...
  const lookupWord = async (word) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, messages: [{ role: "user", content: `You are an English vocabulary assistant for a Serbian speaker at B1 level learning English.\n\nFor the word or phrase: "${word.trim()}"\n\nRespond ONLY with a JSON object, no markdown, no explanation, exactly this structure:\n{\n  "translation": "prevod na srpski (kratko i jasno)",\n  "partOfSpeech": "noun / verb / adjective / phrase / etc",\n  "sentences": [\n    { "en": "First example sentence using the word naturally in context.", "sr": "Prevod prve recenice na srpski." },\n    { "en": "Second example sentence, different context.", "sr": "Prevod druge recenice na srpski." },\n    { "en": "Third example sentence, different context.", "sr": "Prevod trece recenice na srpski." }\n  ]\n}\n\nMake sentences natural, varied in context, B1-B2 level difficulty. Serbian translations should be natural, not literal. Do not include anything outside the JSON.` }] })
      });
      const data = await res.json();
  ...
  const regenerateSentences = async (word, setter, currentSentences) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 800, messages: [{ role: "user", content: `Generate 3 NEW and DIFFERENT example sentences for the English word/phrase: "${word}"\n\nThese sentences were already used, do NOT repeat them:\n${currentSentences?.map(s => "- " + (s.en || s)).join("\n")}\n\nRespond ONLY with a JSON array, no markdown, no explanation:\n[\n  { "en": "First new example sentence.", "sr": "Prevod na srpski." },\n  { "en": "Second new example sentence.", "sr": "Prevod na srpski." },\n  { "en": "Third new example sentence.", "sr": "Prevod na srpski." }\n]\n\nVary the contexts, B1-B2 difficulty. Natural Serbian translations.` }] })
      });
      const data = await res.json();
      const text = data.content?.find(b => b.type === "text")?.text || "";
